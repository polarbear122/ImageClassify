...
会报错：RuntimeError: invalid multinomial distribution (sum of probabilities <= 0)
'''
...
class SMART_block(nn.Module):
    ...
    def __init__(self, in_channel, out_channel, kernel_size=(3, 3, 3), stride=(1, 1, 1), padding=(1, 1, 1)):
        ...
        self.appearance_conv = nn.Conv3d(in_channel, out_channel, kernel_size=(1, kernel_size[1], kernel_size[2]),
                                         stride=stride, padding=(0, padding[1], padding[2]), bias=False)
        self.appearance_bn = nn.BatchNorm3d(out_channel)

        self.relation_conv = nn.Conv3d(in_channel, out_channel, kernel_size=kernel_size, stride=stride, padding=padding,
                                       bias=False)
        self.relation_bn1 = nn.BatchNorm3d(out_channel)
        self.relation_pooling = nn.Conv3d(out_channel, out_channel // 2, kernel_size=1, stride=1,
                                          groups=out_channel // 2, bias=False)
...
class ARTNet(nn.Module):
    ...
    def __init__(self, num_class):
        ...
        self.conv1 = SMART_block(64, 64, kernel_size=(3, 3, 3), stride=(2, 2, 2), padding=(1, 3, 3))
