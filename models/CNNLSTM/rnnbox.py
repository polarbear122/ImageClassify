...
class LeNetVariant(nn.Module):
    def __init__(self):
        ...
        self.features = nn.Sequential(
            nn.Conv2d(3, 16, kernel_size=(5, 5), stride=(1, 1),
                      padding=(2, 2)),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(2, 2),
            nn.Conv2d(16, 32, kernel_size=(5, 5), stride=(1, 1)),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(2, 2))
        self.classifier = nn.Sequential(nn.Linear(3200, 320),
                                        nn.Linear(320, 32))
...
class CNNLSTM(nn.Module):
    def __init__(self, num_classes=2):
        ...
        self.cnn = LeNetVariant()
        self.lstm = nn.LSTM(input_size=32, hidden_size=128, num_layers=2,
                            batch_first=True)
...
def AttBiLSTM_init():
    return AttBiLSTM(2, 84, 256, 2, 0.4)
